-- Atomic registration of Cloudinary media_assets + vehicle_media.
-- Invoker security: relies on existing staff RLS policies.
...
begin
  if p_public_id is null or length(trim(p_public_id)) = 0 then
    raise exception 'public_id requerido';
  end if;
...
comment on function public.register_cloudinary_vehicle_media is
  'Atomically inserts media_assets (cloudinary) + vehicle_media. Staff RLS applies via SECURITY INVOKER.';
